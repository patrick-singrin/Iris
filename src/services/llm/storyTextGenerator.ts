...
function buildStoryUserPrompt(
  ...
  sections.push('')
  sections.push('## Temporal Framing')
  sections.push('IMPORTANT: The event is happening RIGHT NOW or has JUST HAPPENED.')
  sections.push('All generated text must use present tense ("is affected", "is unavailable") or present perfect ("has been resolved").')
  sections.push('NEVER use future tense. Do NOT write "will be affected" or "is scheduled for" — write "is affected" or "has started".')

  sections.push('')
  sections.push('## Placeholder Instructions')
  sections.push('HARD REQUIREMENT: In ALL generated channel text, you MUST replace concrete values with {placeholders}.')
  ...
  sections.push('If the narrative says "March 15", the channel text MUST say "{date}" — NOT "March 15".')
  sections.push('If the narrative mentions a service like "MagentaCLOUD", the channel text MUST say "{service-name}".')
  sections.push('NEVER output concrete dates, times, service names, URLs, or versions in channel text fields.')
